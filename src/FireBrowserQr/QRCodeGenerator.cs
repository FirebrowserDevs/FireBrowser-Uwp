...
namespace FireBrowserQr
{
    public class QRCodeGenerator : IDisposable
    {
        ...
        /// <exception cref="FireBrowserQr.Exceptions.DataTooLongException">Thrown when the payload is too big to be encoded in a QR code.</exception>
        ...
        public static QRCodeData GenerateQrCode(string plainText, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1)
        {
            ...
            else
            {
                ...
                if (minVersion > version)
                {
                    ...
                    throw new FireBrowserQr.Exceptions.DataTooLongException(eccLevel.ToString(), encoding.ToString(), version, maxSizeByte);
                }
            }
        ...
        private static int GetVersion(int length, EncodingMode encMode, ECCLevel eccLevel)
        {
            ...
            throw new FireBrowserQr.Exceptions.DataTooLongException(eccLevel.ToString(), encMode.ToString(), maxSizeByte);
        }
